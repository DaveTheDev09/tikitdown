...
// ----- toast -----
    ...
    function buildDownloadUrl(qualityKey, chosen) {
      ...
      if (
        ...
        (qualityKey === 'best' || qualityKey.indexOf('p') !== -1)
      ) {
        return {
          url: chosen.cdn_url,
          filename: filename,
    ...
    function clearResults() {
      ...
      if (dlPreview) {
        dlPreview.innerHTML = '';
        dlPreview.setAttribute('hidden', '');
      }
      if (dlActions) {
        dlActions.innerHTML = '';
        dlActions.setAttribute('hidden', '');
      }
      if (dlImageGrid) {
    ...
    function renderPreview(data) {
      ...
      dlPreview.innerHTML =
        (thumb ? '<img class="dl-preview__thumb" src="' + escHtml(thumb) + '" alt="' + escHtml(td('videoThumbnail')) + '" loading="lazy">' : '') +
        '<div class="dl-preview__meta">' + author + desc + '<div class="dl-preview__sub">' + sub + '</div></div>';
      dlPreview.removeAttribute('hidden');
    ...
    function renderQualityDropdown() {
      ...
      var srcW = best && best.width || 0;
      var srcSize = best && best.filesize || 0;
      var duration = downloadUiState.previewData && downloadUiState.previewData.duration || 0;

      var fakeLadder = [];
      if (srcH > 0) {
        [1080, 720, 480, 360, 240].forEach(function (h) {
          if (h < srcH) {
            var est = srcSize > 0
              ? Math.max(1, Math.round(srcSize * Math.pow(h / srcH, 1.2)))
              : (duration > 0 ? Math.max(1, Math.round(250 * 1000 * duration / 8 / (srcH / h))) : 0);
            fakeLadder.push({ key: h + 'p', label: h + 'p', size: est });
          }
        });
      }

      var select = document.createElement('select');
      select.className = 'dl-quality-select';
      select.id = 'dlQualitySelect';

      if (best) {
        var origLabel = td('downloadVideo') + (srcH ? ' (' + srcH + 'p)' : '');
        var opt = document.createElement('option');
        opt.value = 'best';
        opt.textContent = origLabel + ' \u00b7 ' + formatBytes(srcSize);
        select.appendChild(opt);

        fakeLadder.forEach(function (q) {
          var o = document.createElement('option');
          o.value = q.key;
          o.textContent = td('downloadVideo') + ' ' + q.label + ' \u00b7 ~' + formatBytes(q.size);
      ...
      if (audio) {
        ...
        ao.value = 'audio';
        ao.textContent = td('downloadAudio') + ' (MP3) \u00b7 ' + formatBytes(audio.filesize);
        select.appendChild(ao);
      }

      var wrapper = document.createElement('div');
      wrapper.className = 'dl-quality-row';
      wrapper.appendChild(select);

      var btn = makeActionElement('dl-video-btn', td('downloadVideo'), function (el) {
        var val = select.value;
        if (!val) return;
        triggerDownload(val, el, td('downloadVideo'));
      });

      wrapper.appendChild(btn);
      dlActions.appendChild(wrapper);
