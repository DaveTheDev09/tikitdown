...
// ----- toast -----
    ...
    function renderDownloadButtons() {
      ...
      if (best) {
        var origBtn = makeActionElement('dl-video-btn', td('downloadVideo'), function (el) {
          triggerDownload('best', el, td('downloadVideo'));
        });
      ...
      if (best && srcH >= 720) {
        ...
        hdBtn.tabIndex = 0;
        hdBtn.innerHTML = '<span class="dl-hd-badge">HD</span>' + td('downloadVideo') + ' HD <span class="dl-hd-ad-label">Watch Ad</span>';
        hdBtn.addEventListener('click', function (e) {
          e.preventDefault();
          if (hdBtn.classList.contains('is-busy')) return;
          showAdOverlay(function () {
            triggerDownload('720p', hdBtn, td('downloadVideo') + ' HD');
          });
        });
