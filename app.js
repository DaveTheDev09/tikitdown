...
// ----- toast -----
    ...
    // ----- inline step indicator (inside download card) -----
    var inlineStepsEl = null;

    function showInlineSteps(steps) {
      hideInlineSteps();
      // Insert into the result card so it's visible
      var container = dlResultCard || dlActions;
      if (!container) return;
      inlineStepsEl = document.createElement('div');
      inlineStepsEl.className = 'dl-inline-steps';
      steps.forEach(function (step, i) {
        var item = document.createElement('div');
        item.className = 'dl-inline-step';
        item.innerHTML =
          '<span class="dl-inline-step-num"><span>' + (i + 1) + '</span></span>' +
          '<span class="dl-inline-step-text">' + step + '</span>' +
          '<span class="dl-inline-step-spinner" style="display:none"></span>';
        inlineStepsEl.appendChild(item);
      });
      // Show the card if hidden, and insert steps at the top
      if (dlResultCard && dlResultCard.hasAttribute('hidden')) {
        dlResultCard.removeAttribute('hidden');
      }
      container.insertBefore(inlineStepsEl, container.firstChild);
    }

    function setInlineStepActive(index) {
      if (!inlineStepsEl) return;
      var items = inlineStepsEl.querySelectorAll('.dl-inline-step');
      items.forEach(function (item, i) {
        var spinner = item.querySelector('.dl-inline-step-spinner');
        if (i < index) {
          item.className = 'dl-inline-step done';
          if (spinner) spinner.style.display = 'none';
        } else if (i === index) {
          item.className = 'dl-inline-step active';
    ...
    function hideInlineSteps() {
      if (inlineStepsEl && inlineStepsEl.parentNode) {
        inlineStepsEl.parentNode.removeChild(inlineStepsEl);
      }
      inlineStepsEl = null;
    }
    ...
    function buildTitleFilename(qualityLabel) {
      var desc = downloadUiState.previewData && downloadUiState.previewData.description || '';
      var title = desc.replace(/\n+/g, ' ').trim().slice(0, 80);
      if (!title) {
        title = getFilenamePrefix(downloadUiState.url).replace(/_$/, '') + filenameId(downloadUiState.url);
      }
      var safe = title.replace(/[<>:"/\\|?*]+/g, '').replace(/\s+/g, '_').replace(/^_+|_+$/g, '');
      if (qualityLabel) safe += '_' + qualityLabel;
      return safe + '.mp4';
    }
    ...
    function buildDownloadUrl(qualityKey, chosen) {
      ...
      } else if (qualityKey === 'best') {
        filename = buildTitleFilename('HD');
      } else {
        filename = buildTitleFilename('SD');
      }
    ...
    function setDownloadButtonLoading(loading) {
      ...
      if (loading) {
        ...
        if (label) {
          ...
          label.innerHTML = '<span class="dl-btn-spinner"></span> Fetching video...';
        }
      } else {
    ...
    function clearResults() {
      ...
      // Remove "Download Another Video" link
      var anotherLink = document.querySelector('.dl-another-link');
      if (anotherLink && anotherLink.parentNode) {
        anotherLink.parentNode.removeChild(anotherLink);
      }
      // Remove ad overlay
      if (adOverlayEl && adOverlayEl.parentNode) {
      ...
      var inputMeta = document.querySelector('.input-meta');
      var shareStrip = document.querySelector('.share-strip');
      var formatsRow = document.querySelector('.formats');
      var dlStatusEl = document.getElementById('dlStatus');
      if (inputMeta) inputMeta.style.display = '';
      if (shareStrip) shareStrip.style.display = '';
      if (formatsRow) formatsRow.style.display = '';
      if (dlStatusEl) dlStatusEl.style.display = '';
    }
    ...
    function renderPreview(data) {
      ...
      if (thumbImg) {
        thumbImg.onerror = function () {
          if (this.dataset.triedRaw) { this.onerror = null; return; }
          var raw = this.getAttribute('data-raw-src');
          if (raw) { this.dataset.triedRaw = '1'; this.src = raw; }
          else { this.onerror = null; }
        };
      }
      if (dlResultCard) dlResultCard.removeAttribute('hidden');
      setPhoneBackground(rawThumb || thumb);
    }
    ...
    function setActionButtonState(button, active, pct, restoreText) {
      ...
      // HD button: only update progress, never rebuild innerHTML (preserves badge/label)
      if (button.classList.contains('dl-hd-btn')) {
        if (active) {
          button.classList.add('is-busy');
          button.setAttribute('aria-disabled', 'true');
          // Update progress text if present
          var labelSpan = button.querySelector('.dl-btn-label');
          if (labelSpan) labelSpan.textContent = pct != null ? td('downloading') + ' ' + pct + '%' : td('downloading') + '...';
          // Update progress bar if present
          var progressBar = button.querySelector('.dl-btn-progress');
          if (progressBar) progressBar.style.width = (pct || 0) + '%';
        } else {
          button.classList.remove('is-busy');
          button.setAttribute('aria-disabled', 'false');
          // Remove spinner
          var spinner = button.querySelector('.dl-btn-spinner');
          if (spinner) spinner.remove();
          // Success flash
          var success = document.createElement('div');
          success.className = 'dl-btn-success';
          button.style.position = 'relative';
          button.style.overflow = 'hidden';
          button.appendChild(success);
          setTimeout(function () { if (success.parentNode) success.remove(); }, 700);
        }
        return;
      }

      // If already active, just update progress bar (no rebuild)
      if (active && button.classList.contains('is-busy')) {
        var progressBar = button.querySelector('.dl-btn-progress');
        if (progressBar) progressBar.style.width = (pct || 0) + '%';
        var labelSpan = button.querySelector('.dl-btn-label');
        if (labelSpan) labelSpan.textContent = pct != null ? td('downloading') + ' ' + pct + '%' : td('downloading') + '...';
      ...
      // Remove old overlays
      var oldProgress = button.querySelector('.dl-btn-progress');
      var oldSpinner = button.querySelector('.dl-btn-spinner');
      var oldSuccess = button.querySelector('.dl-btn-success');
      ...
      if (active) {
        ...
        // Build new content: spinner + text + progress bar
        var labelText = pct != null ? td('downloading') + ' ' + pct + '%' : td('downloading') + '...';
        button.innerHTML =
          '<span class="dl-btn-spinner"></span>' +
          '<span class="dl-btn-label">' + labelText + '</span>' +
          '<div class="dl-btn-progress" style="width:' + (pct || 0) + '%"></div>';
      } else {
    ...
    function triggerDownload(qualityKey, button, restoreText) {
      ...
      function onProgress(pct) {
        ...
        if (pct > 10) setInlineStepActive(1);
      }
      ...
      fetchAndSave(built.url, built.filename, onProgress, onBytes).catch(function (err) {
        ...
        setTimeout(hideInlineSteps, 1500);
    ...
    function renderDownloadButtons() {
      ...
      // "Download Another Video" link (outside card, like snaptik)
      var anotherBtn = document.createElement('div');
      anotherBtn.className = 'dl-another-link';
      anotherBtn.textContent = '\u21bb Download Another Video';
      anotherBtn.addEventListener('click', function () {
        clearResults();
        urlInput.value = '';
        urlInput.focus();
      });
      if (dlResultCard && dlResultCard.parentNode) {
        dlResultCard.parentNode.insertBefore(anotherBtn, dlResultCard.nextSibling);
      }
    ...
    function fetchInfo() {
      ...
      // Show inline steps in card
      showInlineSteps(['Fetching video info...', 'Extracting download links', 'Ready to download']);
      setInlineStepActive(0);
      ...
      fetch(apiBase + endpoint, {
        ...
        renderImageGrid();
        // Hide extra elements below card (snaptik-style clean result)
        var inputMeta = document.querySelector('.input-meta');
        var shareStrip = document.querySelector('.share-strip');
        var formatsRow = document.querySelector('.formats');
        var dlStatusEl = document.getElementById('dlStatus');
        if (inputMeta) inputMeta.style.display = 'none';
        if (shareStrip) shareStrip.style.display = 'none';
        if (formatsRow) formatsRow.style.display = 'none';
        if (dlStatusEl) dlStatusEl.style.display = 'none';
        // Mark inline steps done
        setInlineStepActive(1);
        setTimeout(function () { setInlineStepActive(2); }, 400);
        setTimeout(hideInlineSteps, 1500);
        // Scroll to result card
        if (dlResultCard) {
          setTimeout(function () {
            dlResultCard.scrollIntoView({ behavior: 'smooth', block: 'center' });
          }, 300);
        }
